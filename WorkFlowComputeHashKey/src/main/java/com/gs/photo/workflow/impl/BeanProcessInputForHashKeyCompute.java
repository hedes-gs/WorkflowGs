...
public class BeanProcessInputForHashKeyCompute implements IProcessInputForHashKeyCompute {
    ...
    protected void processIncomingFile() {
        ...
        this.LOGGER.debug("Subscribing done on topic {}", this.topicScanOutput);
        ...
        while (true) {
            try {
                ...
                records.forEach((r) -> {
                    try {
                        byte[] rawFile = this.beanImageFileHelper.readFirstBytesOfFile(r.key(), r.value());
                        this.LOGGER
                            .info("[EVENT][{}] getting bytes to compute hash key, length is ", r.key(), rawFile.length);
                        String key = this.beanImageFileHelper.computeHashKey(rawFile);
                        this.igniteDAO.save(key, rawFile);
                        this.LOGGER.info("[EVENT][{}] saved in ignite {} ", key, r.value() + "@" + r.key());
                        this.producerForPublishingOnStringTopic.send(
                            new ProducerRecord<String, String>(this.topicHashKeyOutput,
                                key,
                                r.value() + File.pathSeparator + r.key()));
                        metrics.put(key, r.key());
                    } catch (IOException e) {
                        this.LOGGER.error("[EVENT][{}] Error when processing key", r.key(), e);
                    }
                });
                ...
                metrics.entrySet()
                    .forEach(
                        (e) -> {
                            this.LOGGER.info("[EVENT][{}] Compute hashkey for path {}", e.getKey(), e.getValue());
                        });

            } catch (InterruptException e) {
                this.LOGGER.error("[EVENT][{}] Error when processing ", e);
                break;
            } catch (Exception e) {
                this.LOGGER.error("[EVENT][{}] Error when processing ", e);

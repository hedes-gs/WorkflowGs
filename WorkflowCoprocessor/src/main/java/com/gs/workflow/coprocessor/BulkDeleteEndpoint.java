...
/**
 * Defines a protocol to delete data in bulk based on a scan. The scan can be
 * range scan or with conditions(filters) etc.This can be used to delete rows,
 * column family(s), column qualifier(s) or version(s) of columns.When delete
 * type is FAMILY or COLUMN, which all family(s) or column(s) getting deleted
 * will be determined by the Scan. Scan need to select all the
 * families/qualifiers which need to be deleted.When delete type is VERSION,
 * Which column(s) and version(s) to be deleted will be determined by the Scan.
 * Scan need to select all the qualifiers and its versions which needs to be
 * deleted.When a timestamp is passed only one version at that timestamp will be
 * deleted(even if Scan fetches many versions). When timestamp passed as null,
 * all the versions which the Scan selects will get deleted.
 *
 * <br>
 * Example:
 * 
...
public class BulkDeleteEndpoint extends BulkDeleteService implements RegionCoprocessor {
    ...
    public void delete(RpcController controller, BulkDeleteRequest request, RpcCallback<BulkDeleteResponse> done) {
        ...
        try {
            ...
            while (hasMore) {
                ...
                if (deleteRows.size() > 0) {
                    ...
                    for (List<Cell> deleteRow : deleteRows) {
                        deleteArr[i++] = this.createDeleteMutation(deleteRow, deleteType, timestamp);
                    }
                    ...
                    for (i = 0; i < opStatus.length; i++) {
                        ...
                        if (deleteType == DeleteType.VERSION) {
                            byte[] versionsDeleted = deleteArr[i]
                                .getAttribute(BulkDeleteEndpoint.NO_OF_VERSIONS_TO_DELETE);
            ...
            BulkDeleteEndpoint.LOG.error(ioe.toString(), ioe);
            // Call ServerRpcController#getFailedOn() to retrieve this IOException at client
            // side.
        ...
        if (deleteType == DeleteType.FAMILY) {
            ...
            for (Cell kv : deleteRow) {
                ...
                if (columns.add(column)) {
                    // Making deleteColumns() calls more than once for the same cf:qualifier is not
                    // correct
                    // Every call to deleteColumns() will add a new KV to the familymap which will
                    // finally
            ...
            // When some timestamp was passed to the delete() call only one version of the
            // column (with
            // given timestamp) will be deleted. If no timestamp passed, it will delete N
            // versions.
            // How many versions will get deleted depends on the Scan being passed. All the
            // KVs that
            ...
            delete.setAttribute(BulkDeleteEndpoint.NO_OF_VERSIONS_TO_DELETE, Bytes.toBytes(noOfVersionsToDelete));
        }
        return delete;
    }

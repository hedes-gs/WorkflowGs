...
public class FileChannelDataInput implements DataInput, Closeable {
	...
	public FileChannelDataInput(
			byte[] content) {
		this.mappedByteBuffer = ByteBuffer.wrap(content);
	}

	public void setFileChannel(FileChannel fileChannel) {
		try {
			this.mappedByteBuffer = fileChannel.map(MapMode.READ_ONLY,
					0,
					FileChannelDataInput.MAPPED_MEMORY_SIZE);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public boolean readBoolean() throws IOException {
		return this.mappedByteBuffer.get() == 0;
	}
	...
	public int readUnsignedShort() throws IOException {
		return (this.mappedByteBuffer.getShort()) & 0xfff;
	}
	...
	public float readFloat() throws IOException {
		return this.mappedByteBuffer.getFloat();
	}
	...
	public void position(int streamHead) {
		this.mappedByteBuffer.position(streamHead);
	}

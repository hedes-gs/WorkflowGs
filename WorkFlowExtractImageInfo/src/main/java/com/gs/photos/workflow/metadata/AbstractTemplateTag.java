...
public abstract class AbstractTemplateTag {
	...
	private void explore(IFD tiffIFD2, List<IFD> allIfds) {
		...
		tiffIFD2.getAllChildren().forEach((ifd) -> this.explore(ifd,
				allIfds));
	}
	...
	public int createSimpleTiffFields(FileChannelDataInput rin, int offset) {
		...
		try {
			...
			for (
				...
				SimpleAbstractField<?> saf = SimpleFieldFactory.createField(type,
						field_length,
						offset);
				saf.updateData(rin);
				final TiffField<?> tiffField = saf.createTiffField(ftag,
						tagValue);
				this.tiffIFD.addField(tiffField);
				AbstractTemplateTag tagTemplate = TemplateTagFactory.create(ftag,
						this.tiffIFD,
						saf);

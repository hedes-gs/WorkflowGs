...
public enum GPSTag implements Tag {
	// EXIF GPSSubIFD tags
	GPS_VERSION_ID("GPS Version ID", (short) 0x0000), GPS_LATITUDE_REF("GPS Latitude Ref",
			(short) 0x0001), GPS_LATITUDE("GPS Latitude", (short) 0x0002) {
				@Override
				public String getFieldAsString(Object value) {
					int[] intValues = (int[]) value;
					if (intValues.length != 6)
						throw new IllegalArgumentException(
								"Wrong number of GPSLatitute data number: " + intValues.length);
					// formatting numbers up to 3 decimal places in Java
					DecimalFormat df = new DecimalFormat("#,###,###.###");
					return StringUtils.rationalToString(df, true, intValues[0], intValues[1]) + '\u00B0'
							+ StringUtils.rationalToString(df, true, intValues[2], intValues[3]) + "'"
							+ StringUtils.rationalToString(df, true, intValues[4], intValues[5]) + "\"";
				}
			},
	GPS_LONGITUDE_REF("GPS Longitude Ref", (short) 0x0003), GPS_LONGITUDE("GPS Longitude", (short) 0x0004) {
		@Override
		public String getFieldAsString(Object value) {
			int[] intValues = (int[]) value;
			if (intValues.length != 6)
	...
	GPS_ALTITUDE_REF("GPS Altitude Ref", (short) 0x0005), GPS_ALTITUDE("GPS Altitude", (short) 0x0006) {
		@Override
		public String getFieldAsString(Object value) {
			int[] intValues = (int[]) value;
	...
	GPS_SATELLITES("GPS Satellites", (short) 0x0008), GPS_STATUS("GPS Status", (short) 0x0009), GPS_MEASURE_MODE(
			"GPS Measure Mode", (short) 0x000a), GPS_DOP("GPS DOP/Processing Software",
					(short) 0x000b), GPS_SPEED_REF("GPS Speed Ref", (short) 0x000c), GPSSpeed("GPS Speed",
							(short) 0x000d), GPS_TRACK_REF("GPS Track Ref", (short) 0x000e), GPS_TRACK("GPS Track",
									(short) 0x000f), GPS_IMG_DIRECTION_REF("GPS Img Direction Ref",
											(short) 0x0010), GPS_IMG_DIRECTION("GPS Img Direction", (short) 0x0011) {
												@Override
												public String getFieldAsString(Object value) {
													int[] intValues = (int[]) value;
													if (intValues.length != 2)
														throw new IllegalArgumentException(
																"Wrong number of GPSImgDirection data number: "
																		+ intValues.length);
													// formatting numbers up to 3 decimal places in Java
													DecimalFormat df = new DecimalFormat("#,###,###.###");
	...
	GPS_MAP_DATUM("GPS Map Datum", (short) 0x0012), GPS_DEST_LATITUDE_REF("GPS Dest Latitude Ref",
			(short) 0x0013), GPS_DEST_LATITUDE("GPS Dest Latitude", (short) 0x0014) {
				@Override
				public String getFieldAsString(Object value) {
					int[] intValues = (int[]) value;
					if (intValues.length != 6)
						throw new IllegalArgumentException(
								"Wrong number of GPSDestLatitute data number: " + intValues.length);
					// formatting numbers up to 3 decimal places in Java
					DecimalFormat df = new DecimalFormat("#,###,###.###");
					return StringUtils.rationalToString(df, true, intValues[0], intValues[1]) + '\u00B0'
							+ StringUtils.rationalToString(df, true, intValues[2], intValues[3]) + "'"
	...
	GPS_DEST_LONGITUDE_REF("GPS Dest Longitude Ref", (short) 0x0015), GPS_DEST_LONGITUDE("GPS Dest Longitude",
			(short) 0x0016) {
		@Override
		public String getFieldAsString(Object value) {
			int[] intValues = (int[]) value;
	...
	GPS_DEST_BEARING_REF("GPS Dest Bearing Ref", (short) 0x0017), GPS_DEST_BEARING("GPS Dest Bearing", (short) 0x0018) {
		@Override
		public String getFieldAsString(Object value) {
			int[] intValues = (int[]) value;
			if (intValues.length != 2)
	...
	GPS_DEST_DISTANCE_REF("GPS Dest Distance Ref", (short) 0x0019), GPS_DEST_DISTANCE("GPS Dest Distance",
			(short) 0x001a) {
		@Override
		public String getFieldAsString(Object value) {
			int[] intValues = (int[]) value;
	...
	GPS_PROCESSING_METHOD("GPS Processing Method", (short) 0x001b), GPS_AREA_INFORMATION("GPS Area Information",
			(short) 0x001c), GPS_DATE_STAMP("GPS Date Stamp", (short) 0x001d), GPS_DIFFERENTIAL("GPS Differential",
					(short) 0x001e), GPS_HPOSITIONING_ERROR("GPS H Positioning Error", (short) 0x001f),
	// unknown tag

...
public class StringUtils {
	...
	public static String byteArrayToHexString(byte[] bytes, int offset, int length) {
		...
		for (int i = offset; i < endOffset; i++) {
			hex.append("0x").append(HEXES[(bytes[i] & 0xf0) >> 4]).append(HEXES[bytes[i] & 0x0f]).append(",");
		}
	...
	/**
	 * Regular expression version of the String contains method. If used with a
	 * match from start or match from end regular expression, it becomes the regular
	 * expression version of the {@link String# startsWith(String)} or
	 * {@link String#endsWith(String)} methods.
	 *
	 * @param input
	 *            the input string
	 * @param regex
	...
	public static String decimalToDMS(double coord) {
		...
		//
		// next get the integer part of the coord. In other words the, whole number
		// part.
		// e.g. intPart = 87
		double mod = coord % 1;
		degrees = (int) coord;
	 ...
	 *
	 * Input: latitude or longitude in the DMS format ( example: N 43Ã‚Â° 36'
	 * 15.894")
	 *
	...
	/**
	 * Escapes HTML reserved characters and other characters which might cause Cross
	 * Site Scripting (XSS) hacks
	 *
	 ...
	 *
	 * @param queryString
	 *            a URL string with a query string attached
	 ...
	 *
	 * @param str
	 *            the input CharSequence to check
	 ...
	 *            an array of int.
	 * @param unsigned
	 *            true if the int value should be treated as unsigned, otherwise
	 *            false
	...
	/**
	 * Replaces the last occurrence of the string represented by the regular
	 * expression
	 *
	 * @param input
	 *            input string
	 ...
	 *            the regular expression to which this string is to be matched
	 * @param replacement
	 *            the string to be substituted for the match
	 ...
	 *            an array of short.
	 * @param unsigned
	 *            true if the short value should be treated as unsigned, otherwise
	 *            false
	...
	public static Iterator<Character> stringIterator(final String string) {
		...
		return new Iterator<Character>() {
			...
			public Character next() {
				/*
				 * Throw NoSuchElementException as defined by the Iterator contract, not
	 ...
	 * @return a string with the specified characters replaced by HTML entities and
	 *         HTML tags
	 */
	...
	public static String toHexString(byte[] bytes, int offset, int length) {
		...
		for (int i = offset; i < endOffset; i++) {
			hex.append(HEXES[(bytes[i] & 0xf0) >> 4]).append(HEXES[bytes[i] & 0x0f]);
		}

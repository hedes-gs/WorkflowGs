...
public class GsFlux {
	 ...
	 * @throws Exception
	 *             if parsing/topology creation fails
	 */
	public static void main(String[] args) throws Exception {
		...
		options.addOption(
			option(
				...
				"l",
				OPTION_LOCAL,
				"Ignored: to run in local mode use `storm local`" + " instead of `storm jar`"));
		...
		options.addOption(
			option(
				...
				"r",
				OPTION_REMOTE,
				"Ignored: to run on a remote cluster launch"
		...
		options.addOption(
			option(
				...
				OPTION_SLEEP,
				"ms",
				"Ignored: to set cluster run time" + " use `--local-ttl` with `storm local` instead."));
		...
		options.addOption(
			option(
				...
				"d",
				OPTION_DRY_RUN,
				"Do not run or deploy the topology. Just build, validate, "
		...
		options.addOption(
			option(
				...
				"q",
				OPTION_NO_DETAIL,
				"Suppress the printing of topology details."));
		...
		options.addOption(
			option(
				...
				"n",
				OPTION_NO_SPLASH,
				"Suppress the printing of the splash screen."));
		...
		options.addOption(
			option(
				...
				"i",
				OPTION_INACTIVE,
				"Deploy the topology, but do not activate it."));
		...
		options.addOption(
			option(
				...
				"e",
				OPTION_ENV_FILTER,
				"Perform environment variable substitution. Replace keys"
		...
		if (cmd.getArgs().length != 1 || cmd.hasOption(
			OPTION_HELP)) {
			usage(
				options);
			System.exit(
	...
	private static Option option(int argCount, String shortName, String longName, String argName, String description) {
		Option option = OptionBuilder.hasArgs(
			argCount).withArgName(
				argName).withLongOpt(
					longName)
				.withDescription(
					description)
	...
	private static void usage(Options options) {
		...
		formatter.printHelp(
			"storm jar <my_topology_uber_jar.jar> " + GsFlux.class.getName() + " [options] <topology-config.yaml>",
			options);
	}
	...
	private static void runCli(CommandLine cmd) throws Exception {
		...
		String filePath = cmd.getArgList().get(
			0);
		...
		if (cmd.hasOption(
			OPTION_FILTER)) {
			filterProps = cmd.getOptionValue(
				OPTION_FILTER);
		}

		Properties properties = null;
		boolean envFilter = cmd.hasOption(
			OPTION_ENV_FILTER);
		if (cmd.hasOption(
			OPTION_RESOURCE)) {
			printf(
				"Parsing classpath resource: %s",
				filePath);
			properties = FluxParser.parseProperties(
				filterProps,
				true);
			topologyDef = FluxParser.parseResource(
				filePath,
				dumpYaml,
				true,
				properties,
			...
			properties = FluxParser.parseProperties(
				filterProps,
				false);
			topologyDef = FluxParser.parseFile(
				filePath,
				dumpYaml,
				true,
				properties,
		...
		Config conf = FluxBuilder.buildConfig(
			topologyDef);
		ExecutionContext context = new ExecutionContext(topologyDef, conf);
		StormTopology topology = FluxBuilder.buildTopology(
			context);
		...
		if (!cmd.hasOption(
			...
			if (cmd.hasOption(
				OPTION_INACTIVE)) {
				LOG.info(
					"Deploying topology in an INACTIVE state...");
				submitOptions = new SubmitOptions(TopologyInitialStatus.INACTIVE);
			} else {
				LOG.info(
					"Deploying topology in an ACTIVE state...");
			...
			StormSubmitter.submitTopologyAs(
				...
				topology,
				submitOptions,
				new StormSubmitter.ProgressListener() {

					@Override
					public void onStart(String srcFile, String targetFile, long totalBytes) {
						print(
							"start " + srcFile + " / " + targetFile + " totalBytes " + totalBytes);
					}

					@Override
					public void onProgress(String srcFile, String targetFile, long onProgress, long totalBytes) {
						print(
							"onProgress " + srcFile + " / " + targetFile + " - [ " + onProgress + "/" + totalBytes);
					}

					@Override
					public void onCompleted(String srcFile, String targetFile, long totalBytes) {
						print(
							"onCompleted " + srcFile + " / " + targetFile + " totalBytes " + totalBytes);
	...
	private static void printSplash() throws IOException {
		// banner
		InputStream is = GsFlux.class.getResourceAsStream(
			"/splash.txt");
		if (is != null) {

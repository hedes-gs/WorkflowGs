...
@Component
public class KeywordsAssembler extends RepresentationModelAssemblerSupport<String, EntityModel<String>> {
    ...
    // = this .pageableResolver();

    public KeywordsAssembler() { super(ImageController.class,
        KeywordsAssembler.getClassForConstructor()); }

    @Override
    public EntityModel<String> toModel(String entity) {
        KeywordsAssembler.FACTORY.setUriComponentsContributors(Arrays.asList(this.pageableResolver));
        ...
        try {
            return new EntityModel<String>(entity,
                KeywordsAssembler.FACTORY.linkTo(
                    WebMvcLinkBuilder.methodOn(ImageController.class)
                        .getImagesByKeyword(p, null, entity))

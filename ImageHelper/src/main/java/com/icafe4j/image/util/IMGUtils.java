...
/**
 * This utility class contains static methods to help with image manipulation
 * and IO.
 * <p>
...
public class IMGUtils {
	...
	/**
	 * Check the color depth of the image and if the color depth is within 8 bits,
	 * (i.e., a indexed color image), a color map and an index array for the image
	 * are also created. If the color depth is bigger than 8, only the returned
	 * color depth is relevant.
	 * <p>
	 * This method is not yet completely finished. The colorPalette is currently not
	 * ordered. A sorting method could be used to find the most used color from the
	 * colorFreq array, but more tricks will have to be done in order to re-map the
	 * newPixels array to the ordered colorPalette.
	 *
	 * @param rgbTriplets
	 *            an int array of RGB triplets for the image
	 * @param newPixels
	 *            a byte array to hold the color map indexes for the image
	 * @param colorPalette
	 *            the color map for the image
	 * @return an int array holding the color depth and the transparent color index
	 *         if any
	 */
	...
	public static void correctGamma(int[] rgbColorPalette, byte[] gammaTable) {
		for (int i = 0; i < rgbColorPalette.length; i++) {
			...
			rgbColorPalette[i] = ((rgbColorPalette[i] & 0xff000000) | ((red & 0xff) << 16) | ((green & 0xff) << 8)
					| (blue & 0xff));
		}
	}
	...
	public static BufferedImage createThumbnail(InputStream is) throws IOException {
		...
		return getScaledInstance(original, thumbnailWidth, thumbnailHeight, RenderingHints.VALUE_INTERPOLATION_BICUBIC,
				true);
	}
	 ...
	 *
	 * @param gray
	 *            input gray-scale image array - also as output BW image array
	 * @param mask
	 *            a mask array for transparent pixels - 0 transparent, 1 opaque
	 * @param width
	 *            image width
	 * @param height
	 *            image height
	 * @param threshold
	 *            Bayer threshold matrix used to convert to BW image
	 */
	 ...
	 *            image height
	 * @param threshold
	 *            gray-scale threshold to convert to BW image
	 */
	...
	/**
	 * Dither color image using Bayer threshold matrix. This method sometimes makes
	 * the images look too bright. Gamma correction usually can compensate for this
	 * problem.
	 *
	 * @param rgbTriplet
	 *            input pixels in ARGB format
	 * @param width
	 *            image width
	 * @param height
	 *            image height
	 * @param newPixels
	 *            pixel array after dither
	 * @param no_of_color
	 *            actual number of colors used by the color palette
	 * @param colorPalette
	 *            color palette
	 * @param transparent_index
	 *            transparent color index for the color palette
	 * @param threshold
	 ...
	 * <jef@acme.com>. For simplicity, only forward error diffusion is implemented.
	 *
	 * @param rgbTriplet
	 *            input pixels in ARGB format
	 * @param width
	 ...
	 *            color palette
	 * @param transparent_index
	 *            transparent color index of the color palette
	 */
	...
	public static byte[] easyRGB2CMYK(int[] rgb, boolean hasAlpha) {
		...
		for (int i = 0, index = 0; i < rgb.length; i++) {
			...
			if (tempK == 1.0f)
				cmyk[index++] = cmyk[index++] = cmyk[index++] = 0;
			else {
	...
	/**
	 * Entry point for image filtering operation. Simply delegates to the underlying
	 * ImageFilter.
	 */
	...
	/**
	 * Retrieves RGB values from Raster. This seems to be the fastest way to get RGB
	 * values.
	 *
	 * TYPE_INT_RGB = 1 TYPE_INT_ARGB = 2 TYPE_INT_ARGB_PRE = 3 TYPE_INT_BGR = 4
	 * TYPE_3BYTE_BGR = 5 TYPE_4BYTE_ABGR = 6 TYPE_4BYTE_ABGR_PRE = 7 TYPE_BYTE_GRAY
	 * = 10 TYPE_BYTE_BINARY = 12 TYPE_BYTE_INDEXED = 13 TYPE_USHORT_GRAY = 11
	...
	public static int[] getRGB(BufferedImage image) {
		...
		case DataBuffer.TYPE_INT:
			...
			if (type == BufferedImage.TYPE_INT_ARGB_PRE) {
				...
				LOGGER.warn(
						"### Warning: IMGUtils.getRGB() found custom type BufferedImage, fall back to BufferedImage.getRGB() ###");
		...
		case DataBuffer.TYPE_BYTE:
			...
			if (type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_BYTE_BINARY) {
				...
				for (int i = 0, index = 0; i < imageSize; i++)
					rgbs[i] = (((bpixels[index++] & 0xff) << 16) | ((bpixels[index++] & 0xff) << 8)
							| (bpixels[index++] & 0xff) | ((bpixels[index++] & 0xff) << 24));
				...
				for (int i = 0, index = 0; i < imageSize; i++, index += 4) {
					...
					rgbs[i] = (bpixels[index + 3] & 0xff000000) | ((red & 0xff) << 16) | ((green & 0xff) << 8)
							| (blue & 0xff);
				}
			} else if (type == BufferedImage.TYPE_BYTE_GRAY) {
				for (int i = 0; i < imageSize; i++)
					rgbs[i] = (0xff000000) | ((bpixels[i] & 0xff) << 16) | ((bpixels[i] & 0xff) << 8)
							| (bpixels[i] & 0xff);
			} else {
				LOGGER.warn(
						"### Warning: IMGUtils.getRGB() found custom type BufferedImage, fall back to BufferedImage.getRGB() ###");
	...
	public static int[] getRGB2(BufferedImage image) {
		...
		case DataBuffer.TYPE_BYTE:
			...
			if (type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_BYTE_BINARY) {
				...
				if (numOfBanks == 1) {
					for (int i = 0, index = 0; i < imageSize; i++)
						rgbs[i] = ((bpixels[0][index++] & 0xff) << 24) | (bpixels[0][index++] & 0xff)
								| ((bpixels[0][index++] & 0xff) << 8) | ((bpixels[0][index++] & 0xff) << 16);
				} else { // We assume 4 banks
					for (int i = 0; i < imageSize; i++)
						rgbs[i] = ((bpixels[0][i] & 0xff) << 24) | (bpixels[1][i] & 0xff)
								| ((bpixels[2][i] & 0xff) << 8) | ((bpixels[3][i] & 0xff) << 16);
				}
			} else if (type == BufferedImage.TYPE_3BYTE_BGR) {
				if (numOfBanks == 1) {
					for (int i = 0, index = 0; i < imageSize; i++)
						rgbs[i] = 0xff000000 | (bpixels[0][index++] & 0xff) | ((bpixels[0][index++] & 0xff) << 8)
								| ((bpixels[0][index++] & 0xff) << 16);
				} else { // We assume 3 banks
					for (int i = 0; i < imageSize; i++)
						rgbs[i] = 0xff000000 | (bpixels[0][i] & 0xff) | ((bpixels[1][i] & 0xff) << 8)
								| ((bpixels[2][i] & 0xff) << 16);
				}
			} else if (type == BufferedImage.TYPE_4BYTE_ABGR_PRE) {
				if (numOfBanks == 1) {
					for (int i = 0, index = 0; i < imageSize; i++, index += 4) {
						...
						rgbs[i] = ((bpixels[0][index] & 0xff) << 24) | ((red & 0xff) << 16) | ((green & 0xff) << 8)
								| (blue & 0xff);
					}
				} else { // We assume 4 banks
					for (int i = 0; i < imageSize; i++) {
						...
						rgbs[i] = ((bpixels[0][i] & 0xff) << 24) | ((red & 0xff) << 16) | ((green & 0xff) << 8)
								| (blue & 0xff);
					}
				}
			} else if (type == BufferedImage.TYPE_BYTE_GRAY) {
				for (int i = 0; i < imageSize; i++)
					rgbs[i] = 0xff000000 | ((bpixels[0][i] & 0xff) << 16) | ((bpixels[0][i] & 0xff) << 8)
							| (bpixels[0][i] & 0xff);
			}
			return rgbs;
		case DataBuffer.TYPE_USHORT:
	 ...
	 *            useful in down-scaling cases, where {@code targetWidth} or
	...
	public static ImageType guessImageType(FileChannelDataInput is) throws IOException {
		// Read the first ImageIO.IMAGE_MAGIC_NUMBER_LEN bytes
		byte[] magicNumber = is.peek(ImageIO.IMAGE_MAGIC_NUMBER_LEN);
		ImageType imageType = guessImageType(magicNumber);
	 ...
	 *
	 * @param raster
	 *            WritableRaster for ICC_Profile ColorSpace
	 * @param cm
	 *            ColorModel for ICC_Profile ColorSpace
	...
	/**
	 * Reduces a true color image to an indexed-color image with no_of_color using
	 * "Popularity algorithm".
	 *
	 * @param rgbTriplets
	 *            an int array of RGB triplets for the image
	 * @param colorDepth
	 *            the desired color depth - actual value might be smaller
	 * @param newPixels
	 *            a byte array to hold the color map indexes for the image
	 * @param colorPalette
	 *            the color map for the image
	 * @return a two element int array holding the color depth and the transparent
	 *         color index if any
	 */
	private static int[] reduceColorsPopularity(int[] rgbTriplets, int colorDepth, byte[] newPixels,
		...
		// Find the nearest color for the other colors if there are more colors than
		// no_of_color
		if (colorCount > no_of_color) {
			for (int i = no_of_color; i < colorCount; i++) {
				...
				for (int j = 1; j < no_of_color; j++) {
					err2 = (red - clrPalRed[j]) * (red - clrPalRed[j])
							+ (green - clrPalGreen[j]) * (green - clrPalGreen[j])
	...
	// Color quantization
	public static int[] reduceColors(QuantMethod quantMethod, int[] rgbTriplets, int colorDepth, byte[] newPixels,
			final int[] colorPalette) {
	...
	/**
	 * Reduces a true color image to an indexed-color image with no_of_color using
	 * "Popularity algorithm" followed by Floyd-Steinberg error diffusion dithering.
	 */
	public static int[] reduceColorsDiffusionDither(int[] rgbTriplets, int width, int height, int colorDepth,
	...
	public static int[] reduceColorsDiffusionDither(QuantMethod quantMethod, int[] rgbTriplets, int width, int height,
			int colorDepth, byte[] newPixels, final int[] colorPalette) {
		if (colorDepth > 8 || colorDepth < 1)
			throw new IllegalArgumentException("Invalid color depth " + colorDepth);
		int[] colorInfo = new int[2];
		int colors = 0;
	...
	public static int[] reduceColorsOrderedDither(int[] rgbTriplet, int width, int height, int colorDepth,
			byte[] newPixels, final int[] colorPalette, int[][] threshold) {
		if (colorDepth > 8 || colorDepth < 1)
			throw new IllegalArgumentException("Invalid color depth " + colorDepth);
		int[] colorInfo = new int[2];
	...
	public static int[] reduceColorsOrderedDither(QuantMethod quantMethod, int[] rgbTriplets, int width, int height,
			int colorDepth, byte[] newPixels, final int[] colorPalette, int[][] threshold) {
		if (colorDepth > 8 || colorDepth < 1)
			throw new IllegalArgumentException("Invalid color depth " + colorDepth);
		int[] colorInfo = new int[2];
		int colors = 0;
	 ...
	 *
	 * @param rgb
	 *            input RGB image array (format: ARGBARGBARGB...)
	 * @param imageWidth
	 *            image width
	 * @param imageHeight
	 *            image height
	...
	public static void RGB2CMYK(ICC_ColorSpace cmykColorSpace, int[] rgb, float[][] C, float[][] M, float[][] Y,
			float[][] K, int imageWidth, int imageHeight) {
		DataBuffer db = new DataBufferInt(rgb, rgb.length);
		WritableRaster raster = Raster.createPackedRaster(db, imageWidth, imageHeight, imageWidth,
				new int[] { 0x00ff0000, 0x0000ff00, 0x000000ff }, null);
		ColorSpace sRGB = ColorSpace.getInstance(ColorSpace.CS_sRGB);

		ColorConvertOp cco = new ColorConvertOp(sRGB, cmykColorSpace, null);

		BufferedImage rgbImage = new BufferedImage(new DirectColorModel(24, 0x00ff0000, 0x0000ff00, 0x000000ff), raster,
				false, null);
	...
	public static byte[] RGB2CMYK(ICC_ColorSpace cmykColorSpace, int[] rgb, int imageWidth, int imageHeight,
			boolean hasAlpha) {
		DataBuffer db = new DataBufferInt(rgb, rgb.length);
	...
	public static void RGB2CMYK_Inverted(ICC_ColorSpace cmykColorSpace, int[] rgb, float[][] C, float[][] M,
			float[][] Y, float[][] K, int imageWidth, int imageHeight) {
		DataBuffer db = new DataBufferInt(rgb, rgb.length);
		WritableRaster raster = Raster.createPackedRaster(db, imageWidth, imageHeight, imageWidth,
				new int[] { 0x00ff0000, 0x0000ff00, 0x000000ff }, null);
		ColorSpace sRGB = ColorSpace.getInstance(ColorSpace.CS_sRGB);
	...
	public static float[][] rgb2grayscale(int[] rgb, int imageWidth, int imageHeight) {
		...
		for (int i = 0, index = 0; i < imageHeight; i++) {
			for (int j = 0; j < imageWidth; j++, index++) {
				grayscale[i][j] = (float) (((rgb[index] >> 16) & 0xff) * 0.2126 + ((rgb[index] >> 8) & 0xff) * 0.7152
						+ (rgb[index] & 0xff) * 0.0722 - 128.0);
			}
		}
	...
	// Convert RGB to YCbCr with level shift (minus 128)
	public static void RGB2YCbCr(int[][] red, int[][] green, int[][] blue, float[][] Y, float[][] Cb, float[][] Cr,
	...
	public static void RGB2YCCK_Inverted(ICC_ColorSpace cmykColorSpace, int[] rgb, float[][] Y, float[][] Cb,
			float[][] Cr, float[][] K, int imageWidth, int imageHeight) {
		DataBuffer db = new DataBufferInt(rgb, rgb.length);
		WritableRaster raster = Raster.createPackedRaster(db, imageWidth, imageHeight, imageWidth,
				new int[] { 0x00ff0000, 0x0000ff00, 0x000000ff }, null);
		ColorSpace sRGB = ColorSpace.getInstance(ColorSpace.CS_sRGB);

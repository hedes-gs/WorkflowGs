...
public abstract class Metadata implements MetadataReader, Iterable<MetadataEntry> {
	...
	public static Collection<BufferedImage> extractThumbnails(FileChannelDataInput fcdi, String pathToThumbNail)
			throws IOException {
		// ImageIO.IMAGE_MAGIC_NUMBER_LEN bytes as image magic number
		ImageType imageType = IMGUtils.guessImageType(fcdi);
		Collection<BufferedImage> thumbnails = Collections.emptyList();
		// Delegate thumbnail extracting to corresponding image tweaker.
		switch (imageType) {
		case JPG:
			...
			break;
		case TIFF:
			thumbnails = TIFFTweaker.extractThumbnails(fcdi, pathToThumbNail);
			break;
		case PNG:
			LOGGER.info("PNG image format does not contain any thumbnail");
			break;
	...
	public static void insertICCProfile(InputStream is, OutputStream out, byte[] icc_profile) throws IOException {
		...
		default:
			...
			throw new IllegalArgumentException(
					"ICCProfile data inserting is not supported for " + imageType + " image");
		}
		peekHeadInputStream.shallowClose();
	}
	...
	public static void insertIPTC(InputStream is, OutputStream out, Collection<IPTCDataSet> iptcs, boolean update)
			throws IOException {
		// ImageIO.IMAGE_MAGIC_NUMBER_LEN bytes as image magic number
		PeekHeadInputStream peekHeadInputStream = new PeekHeadInputStream(is, ImageIO.IMAGE_MAGIC_NUMBER_LEN);
		ImageType imageType = IMGUtils.guessImageType(peekHeadInputStream);
	...
	public static void insertIRB(InputStream is, OutputStream out, Collection<_8BIM> bims, boolean update)
			throws IOException {
		// ImageIO.IMAGE_MAGIC_NUMBER_LEN bytes as image magic number
		PeekHeadInputStream peekHeadInputStream = new PeekHeadInputStream(is, ImageIO.IMAGE_MAGIC_NUMBER_LEN);
		ImageType imageType = IMGUtils.guessImageType(peekHeadInputStream);
	...
	public static void insertIRBThumbnail(InputStream is, OutputStream out, BufferedImage thumbnail)
			throws IOException {
		// ImageIO.IMAGE_MAGIC_NUMBER_LEN bytes as image magic number
		PeekHeadInputStream peekHeadInputStream = new PeekHeadInputStream(is, ImageIO.IMAGE_MAGIC_NUMBER_LEN);
		ImageType imageType = IMGUtils.guessImageType(peekHeadInputStream);
	 ...
	 * @param os
	 *            OutputStream for the image with Metadata inserted
	 * @throws IOException
	 */
	public static void insertMetadata(Collection<Metadata> metadata, InputStream is, OutputStream os)
			throws IOException {
		// ImageIO.IMAGE_MAGIC_NUMBER_LEN bytes as image magic number
		PeekHeadInputStream peekHeadInputStream = new PeekHeadInputStream(is, ImageIO.IMAGE_MAGIC_NUMBER_LEN);
		ImageType imageType = IMGUtils.guessImageType(peekHeadInputStream);
		...
		default:
			peekHeadInputStream.close();
			throw new IllegalArgumentException(
					"inserting more than one metadata is not supported for " + imageType + " image");
		}
		peekHeadInputStream.shallowClose();
	}
	 ...
	 *
	 * @param is
	 *            InputStream for the input image
	 * @param os
	 *            OutputStream for the output image
	 * @throws IOException
	 */
	public static void removeMetadata(InputStream is, OutputStream os, MetadataType... metadataTypes)
			throws IOException {
		// ImageIO.IMAGE_MAGIC_NUMBER_LEN bytes as image magic number
		PeekHeadInputStream peekHeadInputStream = new PeekHeadInputStream(is, ImageIO.IMAGE_MAGIC_NUMBER_LEN);
		ImageType imageType = IMGUtils.guessImageType(peekHeadInputStream);
	...
	public Metadata(MetadataType type, byte[] data) {
		...
		if (data.length == 0)
			isDataRead = true; // Allow for zero length data but disable read
		this.type = type;
		this.data = data;
	 ...
	 *
	 * @param out
	 *            OutputStream to write the metadata to
	 * @throws IOException
	 */

...
public class TestExchangedTiffDataSerDeUT {
	...
	public void test001_shouldSerializeAndDeserializeSuccessWithDefaultPojo() {
		...
		builder.withId("<id>").withDataAsByte(new byte[] {}).withDataAsInt(new int[] {}).withDataAsShort(new short[] {})
				.withFieldType(FieldType.ASCII).withKey("<key>").withImageId("<img id>");
		...
		ExchangedTiffData hit = deser.deserialize(null,
				results);
	...
	public void test002_shouldSerializeAndDeserializeSuccessWithValuedPojo() {
		ExchangedDataSerializer ser = new ExchangedDataSerializer();
		final ExchangedTiffData.Builder builder = ExchangedTiffData.builder();
		builder.withId("<id>").withDataAsByte(TestExchangedTiffDataSerDeUT.DATA_AS_BYTE)
				.withDataAsInt(TestExchangedTiffDataSerDeUT.DATA_AS_INT)
				.withDataAsShort(TestExchangedTiffDataSerDeUT.DATA_AS_SHORT).withFieldType(FieldType.ASCII)
				.withKey("<key>").withImageId("<img id>");
		final ExchangedTiffData data = builder.build();
		byte[] results = ser.serialize(null,
				data);

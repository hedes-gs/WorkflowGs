...
public class ConsumerForRecordHbaseExif extends AbstractConsumerForRecordHbase<HbaseData>
    ...
    private static Logger                 LOGGER = LoggerFactory.getLogger(ConsumerForRecordHbaseExif.class);
    ...
    @Override
    protected Optional<WfEvent> buildEvent(HbaseData x) {
    ...
    protected void doRecord(String key, HbaseData h) {
        try {
            if (h instanceof HbaseExifData) {
                HbaseExifData hed = (HbaseExifData) h;
                OffsetDateTime ldt = DateTimeHelper.toLocalDateTime(hed.getCreationDate());
                hed.setRegionSalt((short) ldt.getDayOfMonth());
    ...
    protected void flushAllDAO() throws IOException {
        ...
        try {
            ...
            stopWatch.stop();
            ConsumerForRecordHbaseExif.LOGGER
                .info("[CONSUMER][{}] End of flusing DAOs - time is {} ", this.getConsumer(), stopWatch.formatTime());
        }
    }
